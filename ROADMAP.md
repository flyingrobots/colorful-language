...
- **Delivered consumer honesty:** a standalone zero-dependency Node consumer
  ...
  LSP. Its 239-line IR adapter remains within the reviewed two-times bound
  against the alternatives' combined 275 lines, so stable v1 is retained while
...
| `colorful.syntax/v1` surface IR | Move structured analysis across a process/language boundary. | `colorful ir`, Graft/jedit fixtures, the IR witness, and the standalone independent consumer (#156). | High wire-compatibility cost; the measured IR adapter is 239 lines versus 275 for ANSI and LSP combined. | **Retain stable v1; simplify admission.** Five authenticated wire identities justify current bytes within the reviewed cost bound, not new fields. |
...
4. **Independent consumer value — satisfied:** #156 proves a correctness
   ...
   identities. Its 239-line adapter remains within the reviewed two-times bound
   against the alternatives' combined 275 lines, retaining stable v1 while
