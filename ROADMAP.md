...
| --- | --- | --- |
| **Now — make quality measurable** | LSP capacity and cross-stage analysis costs are explicit; public APIs and workflow security have executable evidence. | Advance #140, then #209, before the remaining validator-confidence queue. Measurements remain advisory until their environment is stable. |
...
ratcheting, the measured LSP capacity envelope, and cross-stage
performance/allocation evidence are now delivered foundations. The active
maturity critical path is runnable public-API evidence
([#140](https://github.com/flyingrobots/colorful-language/issues/140)), followed
by pinned workflow-security analysis
