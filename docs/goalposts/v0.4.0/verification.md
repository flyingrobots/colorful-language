...
| Roadmap inventory | `node scripts/check-roadmap-inventory.mjs`; pull-request CI live inventory with the reconciliation PR number | The historical release-prep check passed on 2026-08-07 with 61 primary markers and 16 open slices while treating #293 as delivered by PR #294. The post-merge structural refresh passes with 62 primary markers. The live 17-open-slice result remains pending until this branch has a pull-request number that can prove [#301](https://github.com/flyingrobots/colorful-language/issues/301) is closed by that exact PR. |
