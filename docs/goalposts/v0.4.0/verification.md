...
| --- | --- | --- |
| Scope reconciliation | `git log --first-parent v0.3.0..origin/main`; open issue and PR inventory; `CHANGELOG.md` ledger | Pass on 2026-08-02. The commit, issue, and pull-request inventory was taken against `main` at `20ae95f71e3807229ad8defacc7eb6cb53e1c85e`: 108 first-parent commits, 24 open issues, zero open pull requests, synchronized 0.4.0 manifests, and 21 Markdown files across 10 topic homes reviewed. The changelog leg is evaluated on the release-prep branch rather than at that head, because `20ae95f` predates the `0.4.0` heading — this change introduces it. Supersedes the 2026-07-29 reading, which recorded 102 commits, 25 open issues, and three open dependency-major pull requests; #274, #276, and #277 were superseded by [#286](https://github.com/flyingrobots/colorful-language/pull/286), and #289 was superseded by [#292](https://github.com/flyingrobots/colorful-language/pull/292) for the same structural reason — a Dependabot branch cannot carry the sibling `fuzz/Cargo.lock` edit. |
| Release packet policy | `node --test scripts/check-release-packet.test.mjs`; `node scripts/check-release-packet.mjs` | Pass on 2026-08-02 at `20ae95f71e3807229ad8defacc7eb6cb53e1c85e`: 49/49 tests; live packet admitted four uniquely labeled goalposts with observable oracles and 34 inline/reference-inventoried scoped issues. The count moved from 33 on 2026-07-30 when [#227](https://github.com/flyingrobots/colorful-language/issues/227) entered scope. |
| Release profile | `bash scripts/release-profile-check.sh` | Pass on 2026-07-29: workspace version 0.4.0. |
| Coverage policy | `node --test scripts/check-coverage-policy.test.mjs` | Pass on 2026-08-02: 25/25 tests. The versioned policy in `.github/coverage-policy.json` is unchanged across the release boundary; exact uncovered-line ceilings ratchet the 94.00% measured workspace baseline under a 92% floor. |
| Roadmap inventory | `node scripts/check-roadmap-inventory.mjs`; live reconciliation against GitHub issues in PR CI | Pass on 2026-08-02: 59 primary markers structurally valid, and every slice closed by the release-prep pull request carries a delivered primary marker. |
| Documentation lint | `npx markdownlint-cli2 '**/*.md'` | Pass on 2026-08-02: 74 files, 0 errors. |
...
| Final preflight | `bash scripts/release-preflight.sh v0.4.0` on clean, aligned `main` | Release scope is disposed as of 2026-08-02: 28 of 30 scoped slices closed, with [#154](https://github.com/flyingrobots/colorful-language/issues/154) and [#37](https://github.com/flyingrobots/colorful-language/issues/37) inherently post-tag, and zero open pull requests. The preflight itself refuses to run outside `main`, so it executes from aligned `main` only after this release-prep change merges. Its result is not yet recorded; no tag or publication is authorized by this witness. |
