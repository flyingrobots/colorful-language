...
- **DIST-9** Every tagged release must publish a machine-readable software bill
  of materials describing the shipped dependency graph, generated by an exact
  pinned tool and covered by the same attestation as the artifacts it
  describes.
