...
- **Independent consumer proof.** The zero-dependency
  ...
  The generated burden ledger measures 249 authored nonblank IR adapter lines,
  including 25 migration-specific lines, versus 49 for ANSI and 305 for LSP
  including its JSON-RPC acquisition client. The schema-derived admission
  runtime is reported separately at 862 unique nonblank lines and 1,724 lines
  across its two committed copies; generated bytes are not presented as an
  authored maintenance reduction. Ledger generation reads both copies and fails
  closed if either is missing or differs byte-for-byte. Because the IR alone
  authenticates all five
  wire identities and its authored adapter is smaller than the alternatives'
  combined 354 lines, the decision is to retain stable v1. This preserves
  current bytes; it does not authorize new contract fields.
...
`scripts/generate-syntax-admission.mjs` parses the GraphQL SDL selected by every
compatibility generation and emits byte-identical, dependency-free JavaScript
admission runtimes for Graft and the standalone consumer. The generated layer
owns exact object fields and presence, primitive and list types, nullability,
enum membership, and signed GraphQL `Int` bounds. A schema mutation therefore
changes runtime admission without a second hand-maintained field or enum table.

The generated layer does not absorb semantic contract rules. Graft and the
standalone consumer retain named checks for source identity, nonnegative and
ordered UTF-8 ranges, token axes, graph ownership and containment, derivation
identity, and output refusal. Generated structural failures map to each
adapter's existing shape category.
`scripts/check-generated-syntax-admission-drift.sh` regenerates into a temporary
directory, compares both committed copies, and runs the generator mutation and
source-policy suite.
...
- Independent admission is deliberately strict: its authored adapter is now
  smaller than ANSI and LSP combined, while the separately reported generated
  runtime remains a substantial review and distribution artifact. Further
  contract expansion remains frozen until separate product evidence justifies
