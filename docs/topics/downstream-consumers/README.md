...
The executable burden ledger measures 249 authored nonblank IR adapter lines
versus 49 for ANSI and 305 for LSP including its JSON-RPC acquisition client.
The structural runtime is generated from each registered GraphQL SDL and
reported separately at 862 unique nonblank lines and 1,724 lines across two
byte-identical committed copies. Generated bytes are not counted as an authored
adapter reduction. Ledger generation reads both committed copies and refuses a
missing or byte-divergent copy. The 13 reviewed generator cases register
through one executable case inventory, and the ledger derives its count from
that authority; missing, extra, or duplicate registrations fail closed. The IR
alone verifies all five wire identities and its authored adapter is now smaller
than the alternatives' combined 354 lines. The decision remains to retain
stable v1 and keep new contract surface frozen until separate product evidence
justifies it.

Graft and the independent package import the same generated structural
admission. Exact fields, field presence, primitive and list types, nullability,
enum membership, and signed GraphQL `Int` bounds come from the
compatibility-selected SDL. Source identity, UTF-8 ranges, token axes, graph
rules, derivation identity, and fail-closed output remain named semantic stages
with the adapters' existing stable error categories.
