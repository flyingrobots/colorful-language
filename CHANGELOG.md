...
- **Monotonic release-witness evidence gates.** The active release packet now
  ...
  the full commit behind that tag, an immutable publish-workflow run, and the
  exact GitHub Release URL; public verification requires dated passed
  ...
  literal-block evidence hidden behind an unavailable state, stale unavailable
  placeholders, malformed identities, and incomplete evidence with
  `E_RELEASE_PACKET_EVIDENCE`; an existing but unreadable branch-base authority
  fails with `E_RELEASE_PACKET_IO`. Historical v0.1.0 through v0.3.0 witnesses
  retain their original readable shapes.
...
- **VS Code extension-host declaration policy.** One reviewed runtime policy
  ...
  lockfile release, an isolated minimum-host API compile, strict TypeScript
  declaration checking, current editor references, and Dependabot automation.
  Deterministic mutations reject declaration drift, unsupported host APIs, a
  missing all-update freeze, weakened compiler checking, or stale
...
- **Verified Homebrew formula release asset.** Tagged releases now derive one
  ...
  it with the editor artifacts. Formula syntax evidence runs on exact Ruby
  3.4.10 through a full-SHA-pinned setup action rather than the runner's ambient
  runtime. The formula installs the synchronized `colorful` and `colorful-lsp`
  binaries without inventing a server CLI version flag. CI, release
  preparation, and tag admission run the same generator and workflow-policy
  mutations. No public tap or `brew install` claim exists yet; #37 retains
  publication, clean-machine upgrade, and rollback proof.
...
- **Independent fuzz dependency updates.** The `/fuzz` Dependabot source now
  ...
  that boundary from every root and fuzz Cargo dependency table, including
  workspace-level declarations, rejects missing, broadened, or substituted
  allow rules, resolves renamed Cargo declarations to their package identities
  before detecting duplicated authority, and keeps the documented fuzz-only
  rollback boundary honest. Root Cargo updates may carry the dependent fuzz
  lockfile needed to keep path-crate evidence synchronized, while the closure
  contract rejects a pull request that mixes root and fuzz manifests.
...
- **VS Code tooling dependency advisory remediation
  ...
  and [GHSA-7p8r-x3mc-p8w7](https://github.com/advisories/GHSA-7p8r-x3mc-p8w7)).**
  ...
  also resolves patched `fast-uri` 3.1.5 without changing the declared
  `@vscode/vsce` release. The supported VS Code floor rises from 1.84 to 1.91 to
  match the client package's declared engine. A deterministic lockfile policy
  rejects every reviewed vulnerable leaf range and an editor/client engine
  mismatch in CI and release preparation.
